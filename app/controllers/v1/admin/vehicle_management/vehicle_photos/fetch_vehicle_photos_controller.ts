...
import ListVehiclePhotosRequestValidator from '#validators/v1/admin/vehicle_management/vehicle_photos/list_vehicle_photos_request_validator'
...
export default class FetchVehiclePhotosController {
  async handle({ request, response }: HttpContext) {
    const { driverIdentifier, page = 1, limit = 10 } = await request.validateUsing(
      ListVehiclePhotosRequestValidator
    )
    ...
    try {
      ...
      const { driverVehiclePhotoPayload: photos, paginationMeta } =
        await DriverVehiclePhotoActions.listDriverVehiclePhotos({
          filterRecordOptionsPayload: { driverId },
