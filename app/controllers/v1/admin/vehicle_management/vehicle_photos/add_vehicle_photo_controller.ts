...
import AddVehiclePhotoRequestValidator from '#validators/v1/admin/vehicle_management/vehicle_photos/add_vehicle_photo_request_validator'
...
export default class AddVehiclePhotoController {
  async handle({ request, response }: HttpContext) {
    const { driverIdentifier, section, photoUrl } = await request.validateUsing(
      AddVehiclePhotoRequestValidator
    )
